...
public class ModChestBlock extends ContainerBlock implements IWaterLoggable, IItemBlock {
  ...
  private static final TileEntityMerger.ICallback<ModChestTileEntity, Optional<INamedContainerProvider>> CALLBACK = new TileEntityMerger.ICallback<ModChestTileEntity, Optional<INamedContainerProvider>>() {
    @Override
    public Optional<INamedContainerProvider> acceptDouble(final ModChestTileEntity iinventory1,
        ModChestTileEntity iinventory2) {
      ...
      return Optional.of(new INamedContainerProvider() {
        ...
        public ITextComponent getDisplayName() {
          if (iinventory1.hasCustomName()) {
            ...
            return iinventory2.hasCustomName() ? iinventory2.getDisplayName()
                : new TranslationTextComponent("container.storage_overhaul.generic_large",
                    iinventory1.getDisplayName());
          }
        }
      });
    }
  ...
  protected ModChestBlock(String name, WoodType woodType, ChestTier tier, String nextTier) {
    super(Block.Properties.of(Material.WOOD).strength(2.5F).sound(SoundType.WOOD));
  ...
  // START CODE ADDED BY TRONKI
  public String getNextTier() {
    return nextTier;
  }
  // END CODE ADDED BY TRONKI

  private Callable renderer = () -> new ChestItemRenderer(woodType, tier);

  @Override
  public Item toItem() {
    return new BlockItem(this, new Item.Properties().tab(ItemGroup.TAB_DECORATIONS).setISTER(() -> renderer))
  ...
  public BlockState updateShape(BlockState stateIn, Direction facing, BlockState facingState, IWorld worldIn,
    ...
    if (facingState.getBlock() == this && facing.getAxis().isHorizontal()) {
      ...
      if (stateIn.getValue(TYPE) == ChestType.SINGLE && chesttype != ChestType.SINGLE
          && stateIn.getValue(FACING) == facingState.getValue(FACING)
  ...
  public BlockState getStateForPlacement(BlockItemUseContext context) {
    ...
    return defaultBlockState().setValue(FACING, direction).setValue(TYPE, chesttype).setValue(WATERLOGGED,
  ...
  private Direction getDirectionToAttach(BlockItemUseContext context, Direction direction) {
    ...
    return blockstate.getBlock() == this && blockstate.getValue(TYPE) == ChestType.SINGLE ? blockstate.getValue(FACING)
        : null;
  }
  ...
  @Nullable
  public static IInventory getInventory(ModChestBlock chestBlock, BlockState blockState, World world, BlockPos blockPos,
      boolean p_226916_4_) {
    return chestBlock.getMergerCallback(blockState, world, blockPos, p_226916_4_).apply(MERGER).orElse(null);
  }

  public TileEntityMerger.ICallbackWrapper<? extends ModChestTileEntity> getMergerCallback(BlockState blockState,
      World world, BlockPos blockPos, boolean b) {
    ...
    return TileEntityMerger.combineWithNeigbour(ModTileEntities.CHEST, ModChestBlock::getType,
        ModChestBlock::getDirectionToAttached, FACING, blockState, world, blockPos, bipredicate);
  }

  @OnlyIn(Dist.CLIENT)
  public static TileEntityMerger.ICallback<ModChestTileEntity, Float2FloatFunction> lidAngleCallback(
      final IChestLid lid) {
  ...
  private static boolean isCatSittingOn(IWorld world, BlockPos pos) {
    List<CatEntity> list = world.getEntitiesOfClass(CatEntity.class,
